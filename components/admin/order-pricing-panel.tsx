...
export function OrderPricingPanel() {
        ...
        <table>
          <thead>
            <tr ${isGST ? 'class="gst-highlight"' : ""}>
              ...
              <th>Cases</th>
              <th>Admin Price (₹)</th>
              ${order.finalPricing ? "<th>Salesman Adjustment (₹)</th><th>Final Price (₹)</th>" : ""}
              <th>Line Total (₹)</th>
              ${isGST ? "<th>GST Status</th><th>GST Number</th>" : ""}
            </tr>
          </thead>
          <tbody>
            ${order.items
              ...
              <tr ${isGST && item.gstBill === "yes" ? 'class="gst-highlight"' : ""}>
                ...
                ${
                  isGST
                    ? `
                  <td>${item.gstBill === "yes" ? "GST Applicable" : "No GST"}</td>
  ...
  const getPriceAdjustmentIndicator = (order: Order) => {
    if (!order.finalPricing || !order.adminPricing) return null

    const totalAdjustment = Object.values(order.finalPricing.adjustments || {}).reduce((sum, adj) => sum + adj, 0)
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        <span className={`text-xs ${totalAdjustment > 0 ? "text-green-600" : "text-red-600"}`}>
                ...
                {filteredOrders.map((order) => {
                  ...
                  return (
                    <TableRow key={order.id} className={isRecentlyModified ? "bg-blue-50" : ""}>
                      ...
                      <TableCell>{getPriceAdjustmentIndicator(order)}</TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1">
                      ...
                      return (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            ...
                            {item.gstBill === "yes" && item.gstNumber && (
                              <p className="text-xs text-muted-foreground mt-1">{item.gstNumber}</p>
                            )}
                          </TableCell>
                        </TableRow>
                      )
